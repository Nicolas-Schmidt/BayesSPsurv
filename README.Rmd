...
- That observations are independent from each other after controlling for covariates. 
...
This model incorporates nonspatial unit-specific i.i.d frailties in the model’s split-stage (Vi) and survival stage (Wi) as well as time-varying covariates in each of these two stages.
...
You can generate the box-plots for unit-specific split and survival-stage frailties from the estimated model as follows. 
```{r, fig.width = 13, fig.height = 10, warning = FALSE}
library(ggplot2)
...
**BayesSPsurv** also fits Bayesian SP survival models without unit-specific i.i.d. frailties.
